...
const PlayerTwo = (props)=>{
    ...
    return (
        <React.Fragment>
        <div onClick={ ()=> props.handlePlayer(props.player.role,props.player.player_index) } className={ props.activeList.includes(props.player.player_index)? "player-container player-orange" : "player-container"}>
            ...
            <div  style={{fontWeight:500,flexGrow:1}}>{ props.activeList.includes(props.player.player_index)? <MdRemoveCircleOutline size={20} style={{color:'orange'}}/> : <MdAddCircleOutline size={20} style={{color:'green'}} /> } 
            </div>
        </div>
        </React.Fragment>
    );
