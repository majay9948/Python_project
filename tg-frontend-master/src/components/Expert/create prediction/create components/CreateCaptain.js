...
const CreateCaptain = (props)=>{
    ...
    let handleAddCaptain = ()=>{
        setTemplate(true)
    }
    let handleRemoveCaptain = ()=>{
        setRequiredData(null)
        toast.success('Captain Player data removed!',{position:'top-center'})
    }
    ...
    let handleSubmit = (e)=>{
        e.preventDefault();
            ...
            {
                toast.warning('Select Captain Players!',{position:'top-center'})
                return;
            }
          let tempList = [...props.predictionData]
          for(let i=0;i<props.predictionData.length;i++){
            if(props.predictionData[i].id === 4){
          ...
          props.setPredictionData(tempList)
          toast.success('Captain Data Added!',{position:'top-center'})
          return;
    }
    ...
    return (
        ...
        { template ?
            ...
            <div style={{padding:5,marginTop:10,marginBottom:10,boxShadow:'0px 0px 5px grey',borderRadius:5}}> 
                <div className='text-center' style={{fontWeight:500}}>Captain Player</div>   
                ...
                <div>
                    <form className="mt-2 p-2" onSubmit={handleSubmit}>
                        <div className='d-flex align-items-center justify-content-center'>
                            {requiredData === null? 
                                <button className='btn btn-success' onClick={()=> handleAddCaptain()} style={{fontWeight:500}}>Add Captain Player</button>
                                : 
                                <div className="expert-block">
                                    <div className='expert-content'>
                                        <div className='expert-box' style={{padding:10,fontWeight:400,fontSize:18,display:'flex',alignItems:'center',justifyContent:'center'}}>
                                            <MdOfflinePin size={24} style={{color:'green'}}/> <span>&nbsp;Data Added</span><button className='btn btn-sm btn-danger' onClick={()=> handleRemoveCaptain()} style={{marginLeft:5,fontWeight:500}}>X</button>
                                        </div>
                                    </div>
                                </div>
                            }
