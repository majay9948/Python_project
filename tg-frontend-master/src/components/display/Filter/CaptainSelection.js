...
const CaptainSelection = (props)=>{
    ...
    useEffect(()=>{
        if(props.responseStuff[1] === 1)
        {
            ...
            props.setSecondFilter(stuff)
            let dp = [...props.filterIndexArray]
            dp[1] = 1
            props.setFilterIndexArray(dp)
            toast.success('Captain Selection Filter Added!',{position:'top-center'})
            return
        }
    },[props.responseStuff])

    useEffect(()=>{
        console.log(props.selectInput)
        console.log(props.secondFilter)
        if(props.selectInput === 1)
        {
            if(props.secondFilter !== null)
            {
                setPresentList([...props.secondFilter.presentList])
            }
        }
    },[props.selectInput])
    ...
    return (
        <React.Fragment>
           <div style={{padding:5,marginTop:10,boxShadow:'0px 0px 5px grey',borderRadius:5}}> 
                <div className='text-center' style={{fontWeight:500}}>Captain Selection</div>   
            ...
            <div>
                ...
                <div class="alert alert-warning mt-2" role="alert">
                    ...
                    <p><b>Player Present</b> will filter teams which contain added players as <b>Captain</b></p>
                </div>
           </div>
        </React.Fragment>
    );
